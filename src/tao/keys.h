...
typedef unique_free_ptr<EVP_PKEY, EVP_PKEY_free> ScopedEvpPkey;

/// A smart pointer to an OpenSSL RSA object.
typedef unique_free_ptr<RSA, RSA_free> ScopedRsa;

/// A smart pointer to an OpenSSL EC_KEY object.
typedef unique_free_ptr<EC_KEY, EC_KEY_free> ScopedEcKey;

/// A smart pointer to an OpenSSL BIO object.
typedef unique_free_ptr<BIO, BIO_free_all> ScopedBio;
...
class Keys {
 ...
 private:
  ...
  /// @{
  unique_ptr<tao::Verifier> verifier_;
  unique_ptr<tao::Signer> signer_;
  unique_ptr<tao::Deriver> deriver_;
  unique_ptr<tao::Crypter> crypter_;
  /// @}
