...
void CDECL vmm_memcpy_assuming_mmio(
    UINT8 *dst,
    UINT8 *src,
    INT32 count)
{
 ...
 * issues.
 */
void *  CDECL vmm_lock_memcpy_ascending(void *dest,
                                        const void* src,
                                        size_t count)
{
    size_t i = 0, j, cnt;
    ...
    {
       	for(i = 0; i < cnt; i++) {
            vmm_lock_xchg_32_64_word(&((VMM_LONG*) d)[i],
                                     &((VMM_LONG*) s)[i]);
        }
...
void *  CDECL vmm_lock_memcpy_descending(void *dest,
                                         const void* src,
                                         size_t count)
{
    size_t i, cnt, rem;
