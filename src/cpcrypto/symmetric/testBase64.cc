...
const char* szA =
    "jf7LKELNZbwta1xSMZCzAjp7WNH9cwIV5LbjLS0kSo8R0QwTSeM4itUiSmLD8RRv+Fj1e/"
    "koAcGlXsXniul//"
    "onUvt5MsxdMiS+DPnfBSf2+H4MwMvn2FRdTkOP9hwt2XlYA4B2JhSLOBK+t2vFHxrcDm0Y9RKP"
    "B5AVTw97tNrnwzYbUdR68ZDU7pHZ/UYZJ/IIkU7ibYr/Olgh+p47Tfu06SBO1ctR9S1MbPHP/"
    "2NA1kQyWsy6Do8ohZOK+XWIG36tI2nLVNATXr3mFj+7XhdGdUMzVzEyvOZE5oXAPmh8d8yfFr2"
    "lrA3nqaFJ+ePmX+X2vEXJKqUjhIMPYmz9Zr/==";
//"AAABAAE=";
...
// const char* szB=
// "";

const char* szB =
    "L5X0f5gsfWWUIObqG2jv1+i4XKXUeL+gixdhy8dOCpLAQQdf3WooRxbQKgCsJlrTsLD0BbrQkh"
    "kex0uQgQ0dN5/e3e8PldebesGUSqasyxj2WgmQnY06m/"
    "HUYUkrOLP7Nb+seAOkq5SiabqFe3kYcPqRE9bVtai37K5vqQz8W5waTPx9b7XYyec4UHplDvcj"
    "EsRMax/"
    "rehPa96cNAix+Kf77LXHX4+D7gVluSK021Rog0SNzP6RglrAAYFDnitAcAwX+iEKcgSaviRzPZ"
    "Es/5tyKnPo0PLLFHQn75lbJPnh2b3S3CNIWczJ39c2BiWG3rgx4UihuNFZLXJZQH2pIYg==";
...
inline byte b64value(char a) {
  if (a >= 'A' && a <= 'Z') return (byte)a - 'A';
  if (a >= 'a' && a <= 'z') return (byte)a - 'a' + 26;
  if (a >= '0' && a <= '9') return (byte)a - '0' + 52;
  if (a == '+') return 0x3e;
  if (a == '/') return 0x3f;
  return 0xff;  // error
}
...
bool AtoBase64(int inLen, const byte* pbIn, int* poutLen, char* szOut,
               bool fDirFwd)
    //
  ...
  // enough room?
  if (numOut > *poutLen) return false;
...
bool AfromBase64(int inLen, const char* szIn, int* poutLen,
                 unsigned char* puOut, bool fDirFwd = true)
    //
    //      Lengths are in characters
    //
    {
  ...
  if (inLen > 2 && *(szIn + inLen - 1) == '=') numOut--;
  if (inLen > 2 && *(szIn + inLen - 2) == '=') numOut--;
  puW = puOut + numOut - 1;
...
bool toBase64(int inlen, const byte* in, int* poutlen, char* szout,
  ...
  if (dir) {
    ...
    // 24 bit chunks
    pb = in + inlen -
         1;  // start at high order byte (eg-on little endian machine)
    while (n > 2) {
      a = ((*pb) >> 2) & 0x3f;
...
bool fromBase64(int inlen, const char* szin, int* poutlen, byte* out,
  ...
  if (inlen < 4 || (inlen % 4) != 0) return false;

  // does padding affect output length?
  if (*(szin + inlen - 1) == '=') numout--;
  if (*(szin + inlen - 2) == '=') numout--;
...
void PrintBytes(int n, byte* rgb) {
  for (int j = 0; j < n; j++) printf("%x", rgb[j]);
  printf("\n");
}
