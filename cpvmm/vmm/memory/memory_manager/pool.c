...
/*INLINE
UINT32 pool_calculate_number_of_free_pages_to_keep(POOL* pool) {
    UINT32 num_of_allocated_pages = pool_get_num_of_allocated_pages(pool);
    UINT32 num = (num_of_allocated_pages / POOL_PAGES_TO_KEEP_THRESHOLD);
...
static
BOOLEAN pool_is_element_in_list(POOL_LIST_HEAD* list_head, POOL_LIST_ELEMENT* element) {
...
static
POOL_LIST_ELEMENT* pool_allocate_node_from_list(POOL_LIST_HEAD* list_head) {
...
static
void pool_insert_node_into_list_at_tail(POOL_LIST_HEAD* list_head, POOL_LIST_ELEMENT* element) {
    UINT32 num_of_elements;
    POOL_LIST_ELEMENT* curr_last_element;

    //VMM_ASSERT(!pool_is_element_in_list(list_head, element));
...
static
void pool_remove_element_from_the_list(POOL_LIST_HEAD* list_head,
...
void pool_free_nodes_in_page_from_list(POOL_LIST_HEAD* list_head,
    ...
    for (covered_size = 0; covered_size < (PAGE_4KB_SIZE - size_of_element + 1); covered_size += size_of_element) {
...
static
void pool_free_several_pages_into_heap(POOL* pool, UINT32 num_of_pages_to_free) {
    POOL_LIST_HEAD* free_pages_list = pool_get_free_pages_list(pool);
...
static
void pool_report_alloc_free_op(POOL* pool) {
...
void* pool_allocate_internal(POOL* pool) {
    ...
    VMM_ASSERT(pool_get_num_of_elements_per_page(pool) > 0); // elements bigger than page are not supported yet
    ...
    pool_split_page_to_elements(free_pool_elements_list, pool_get_size_of_single_element(pool), page);
    VMM_ASSERT(!pool_is_list_empty(free_pool_elements_list));
    ...
    if (!res) {
        ...
        pool_free_nodes_in_page_from_list(free_pool_elements_list, pool_get_size_of_single_element(pool), page);
        pool_dec_num_of_pages_used_for_pool_elements(pool);
        VMM_DEBUG_CODE(vmm_zeromem((void*)page, PAGE_4KB_SIZE));
        pool_insert_node_into_list(free_pages_list, (POOL_LIST_ELEMENT*)page);
        VMM_ASSERT(pool_is_allocation_counters_ok(pool));
        return NULL;
    }
...
POOL_HANDLE pool_create_internal(UINT32 size_of_single_element, BOOLEAN  mutex_flag)
{
    ...
    final_size_of_single_element = (size_of_single_element >= sizeof(POOL_LIST_ELEMENT)) ? size_of_single_element : sizeof(POOL_LIST_ELEMENT);
    ...
    final_size_of_hash_element = (size_of_hash_element >= sizeof(POOL_LIST_ELEMENT)) ? size_of_hash_element : sizeof(POOL_LIST_ELEMENT);
...
void pool_destroy(POOL_HANDLE pool_handle) {
    ...
    VMM_ASSERT(pool_get_num_of_allocated_elements(pool) == 0);
    VMM_ASSERT(hash64_is_empty(pool_get_hash(pool)));
    VMM_ASSERT(pool_get_num_of_allocated_pages(pool) == pool_list_head_get_num_of_elements(pool_get_free_pages_list(pool)));
    hash64_destroy_hash(pool_get_hash(pool));
    VMM_ASSERT(pool_is_list_empty(pool_get_free_hash_elements_list(pool)));
    VMM_ASSERT(pool_is_list_empty(pool_get_free_pool_elements_list(pool)));
