...
BOOLEAN vmexit_vmm_teardown(GUEST_CPU_HANDLE gcpu, VMM_TEARDOWN_PARAMS *vmm_teardown_params)
{
    ...
    if(vmm_teardown_params->is_guest_x64_mode == 1){
        ...
        call_teardown_thunk32(
                ...
                vm_guest_states->ADDR_OF_TEARDOWN_THUNK,
                vmm_teardown_params->cr3_td_sm_32,
                cr4_pae_is_on
                );
    }
    else{
...
static BOOLEAN is_params_ok(GUEST_CPU_HANDLE gcpu, VMM_TEARDOWN_PARAMS *p, UINT64 *state_hva)
{
    ...
    VMM_LOG(
        ...
        (UINT64)p->teardownthunk_gva,
        thunk_hva,
        (UINT64)p->guest_states_storage_virt_addr,
        *state_hva,
        (UINT64)p->cr3_td_sm_32
        );
...
BOOLEAN vmam_add_to_host_page_table(IN GUEST_CPU_HANDLE gcpu,
    ...
    for(i=0; i < num_pages; i++) {
        ...
        result =
            vmam_check_add_one_page_mem_map(
                host_cr3,
                host_cr4,
                efer_value,
                start_gva + i * PAGE_4KB_SIZE,
                gpa + i * PAGE_4KB_SIZE
                );
