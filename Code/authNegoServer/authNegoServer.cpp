//
//  File: authNegoServer.cpp
//      John Manferdelli
//
//  Description: Sever for authNegoServer
//
...
char*   g_szauthCert= NULL;
char*   g_szauthPrivateKey= NULL;
...
bool    initHashes()
{
    ...
    for(int i=0; i<g_iNumHashes; i++) {
        ...
        if(!fromBase64(strlen(AsciiHashes[i].m_szBase64Hash), 
                       ...
                       rgHash)) {
            fprintf(g_logFile, "authNegoServer: can't base64 decode\n");
            return false;
        }
...
#define SERVICENAME             "authNegoServer"
...
KeyInfo* ReadKeyfromFile(const char* szKeyFile)
{
    ...
    if(!pDoc->LoadFile(szKeyFile)) {
        fprintf(g_logFile, "Cant load keyfile %s\n", szKeyFile);
        return NULL;
    }
                             ...
                             char**  pszCert)
{
    ...
    // check quote
    pQuoteKey= keyfromkeyInfo(szquoteKeyInfo);
    if(pQuoteKey==NULL) {
...
#ifdef  TEST
    fprintf(g_logFile, "authNegoServer: checking Evidence List\n");
    fflush(g_logFile);
#endif
    if(szEvidence!=NULL) {
        TiXmlDocument   doc;
        ...
        if(!oEvid.validateEvidenceList(g_pSigningKey, pQuoteKey)) {
...
bool certNego(int fd)
{
...
#ifdef  TEST
    fprintf(g_logFile, "authNegoServer(%d)\n", fd);
#endif
    ...
    try {
        ...
        if((n=getPacket(fd, (byte*)request, MAXREQUESTSIZE, &type, &multi, &final))<0)
            throw  "Can't get packet 1 in authNegoServer\n";

        if(!getDatafromClientCertMessage1(request, &szPolicyKeyId, &szQuote, &szEvidence))
            throw  "Can't decode client packet in authNegoServer\n";
...
#ifdef  TEST
    fprintf(g_logFile, "authNegoServer: socket opened\n");
#endif
...
#ifdef  TEST
    fprintf(g_logFile, "authNegoServer: bind succeeded\n");
#endif
...
// authNegoServer.exe -SigningCert authNegoServer/signingCert.xml -PrivateKey authNegoServer/signingPrivateKey.x
{
    int     i;
    char*   g_szauthCert= NULL;
    char*   g_szauthPrivateKey= NULL;
    ...
    for(i=0;i<an; i++) {
        if(strcmp(av[i], "-SigningCert ")==0) {
            if(an>(i+1))
                g_szauthCert= av[++i];
        }
        if(strcmp(av[i], "-PrivateKey")==0) {
            if(an>(i+1))
                g_szauthPrivateKey= av[++i];
        }
    }

    initLog("authNegoServer.log");
#ifdef  TEST
    fprintf(g_logFile, "authNegoServer\n");
#endif
