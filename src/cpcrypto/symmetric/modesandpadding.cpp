...
void printCBCState(cbc* pMode) {
  ...
  fprintf(g_logFile, "\tEnc %d, mac: %d, pad: %d\n", pMode->m_uEncAlg,
          pMode->m_uMacAlg, pMode->m_uPadAlg);
...
int cbc::lastCipherBlockIn(int size, byte* puIn, byte* puOut)
  ...
  if (!m_fIVValid) {
    fprintf(g_logFile,
            "cbc::lastCipherBlockIn: first cipherblock was not processed\n");
    return -1;
  }

  if (size != expectedsize) {
    fprintf(g_logFile,
            "cbc::lastCipherBlockIn: wrong lastBlock size, got %d bytes\n",
            size);
    return -1;
  }
  ...
  for (residue = m_iBlockSize - 1; residue >= 0; residue--) {
    if (m_rguLastBlocks[residue] != 0) {
      if (m_rguLastBlocks[residue] != 0x80) {
        fprintf(
            g_logFile,
            "cbc::lastCipherBlockIn: bad pad error, %02x, res: %d size: %d\n",
...
//      by f[0]= 0xe1= 11100001, f[1]=0, f[2]=0,..., f[14]==, f[15]=0, f[16]=
// 0x80
//
...
bool gcm::initDec(u32 alg, int keysize, byte* key, int cipherLen, int authLen,
                  int tagLen) {
#ifdef CRYPTOTEST5
  fprintf(g_logFile, "gcm::initDec\n");
#endif
  m_iNumCipherBytes = cipherLen;
  if (!init(alg, keysize, key, tagLen, authLen)) return false;
...
bool gcm::initEnc(u32 alg, int ivSize, byte* rguIV, int keysize, byte* key,
  ...
  m_iNumPlainBytes = plainLen;
  if (!init(alg, keysize, key, tagLen, authLen)) return false;
  if (ivSize != (m_iBlockSize - (int)sizeof(u32))) return false;
  if (rguIV != NULL) {
...
int gcm::getTag(int bufSize, byte* puTag) {
  if (bufSize < m_iTagSize) return -1;
