...
./SigningInstructions --issuer=JohnsCpCa --can_sign=true --isCA=true >> protocol_test.txt
./tpm2_util --command=Flushall
...
./tpm2_util --command=Flushall
./SelfSignPolicyCert --signing_instructions_file=signing_instructions \
--key_file=cloudproxy_key_file --policy_identifier=test-policy-domain --cert_file=policy_key_cert
./SigningInstructions --issuer=JohnsCpCa --can_sign=true >> protocol_test.txt
./tpm2_util --command=Flushall
