...
/*
 * ECDSA: For an ECC system, public key parameters are q,a,b,P (P is called the
* base point); 
 * pick 1 < x < p, x is the private key. Public key is Q = xP. 
 * ECC Encrypt: To encrypt m (already an integer in the right range), map it to
* a point 
 ...
 *  y^2=x^3-3x+b
* 
 *  p256 = ffffffff 00000001 00000000 00000000 00000000 ffffffff ffffffff
* ffffffff
 *  The parameter a = p256 − 3:
 *  a = ffffffff 00000001 00000000 00000000 00000000 ffffffff ffffffff fffffffc
 *  b = 5ac635d8 aa3a93e7 b3ebbd55 769886bc 651d06b0 cc53b0f6 3bce3c3e 27d2604b
 *  xG = 6b17d1f2 e12c4247 f8bce6e5 63a440f2 77037d81 2deb33a0 f4a13945
* d898c296
 *  yG = 4fe342e2 fe1a7f9b 8ee7eb4a 7c0f9e16 2bce3357 6b315ece cbb64068
* 37bf51f5
 *  orderG= ffffffff 00000000 ffffffff ffffffff bce6faad a7179e84 f3b9cac2
* fc632551
...
ECurve nist256curve;
u64 tempp256[4] = {0xffffffff00000001ULL, 0x0000000000000000ULL,
                   0x00000000ffffffffULL, 0xffffffffffffffffULL};
u64 tempa256[4] = {0xffffffff00000001ULL, 0x0000000000000000ULL,
                   0x00000000ffffffffULL, 0xfffffffffffffffcULL};
u64 tempb256[4] = {0x5ac635d8aa3a93e7ULL, 0xb3ebbd55769886bcULL,
                   0x651d06b0cc53b0f6ULL, 0x3bce3c3e27d2604bULL};
u64 tempGx256[4] = {0x6b17d1f2e12c4247ULL, 0xf8bce6e563a440f2ULL,
                    0x77037d812deb33a0ULL, 0xf4a13945d898c296ULL};
u64 tempGy256[4] = {0x4fe342e2fe1a7f9bULL, 0x8ee7eb4a7c0f9e16ULL,
                    0x2bce33576b315eceULL, 0xcbb6406837bf51f5ULL};

u64 temporderG256[4] = {0xffffffff00000000ULL, 0xffffffffffffffffULL,
...
bool initNist() {
  ...
  if (nistinitialized) return true;
  nist256curve.m_bnM = new bnum(4);
  for (i = 0; i < 4; i++) nist256curve.m_bnM->m_pValue[i] = tempp256[3 - i];
  nist256curve.m_bnA = new bnum(4);
  for (i = 0; i < 4; i++) nist256curve.m_bnA->m_pValue[i] = tempa256[3 - i];
  nist256curve.m_bnB = new bnum(4);
  for (i = 0; i < 4; i++) nist256curve.m_bnB->m_pValue[i] = tempb256[3 - i];
  nist256curve.m_bnGx = new bnum(4);
  for (i = 0; i < 4; i++) nist256curve.m_bnGx->m_pValue[i] = tempGx256[3 - i];
  nist256curve.m_bnGy = new bnum(4);
  for (i = 0; i < 4; i++) nist256curve.m_bnGy->m_pValue[i] = tempGy256[3 - i];
