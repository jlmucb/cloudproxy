...
void base64test(const string &data, int enc_len) {
  string b, d;
  EXPECT_TRUE(Base64WEncode(data, &b));
  EXPECT_EQ(enc_len, b.size());
  EXPECT_TRUE(Base64WDecode(b, &d));
  EXPECT_EQ(data, d);
}

TEST(UtilTest, Base64Test) {
  base64test("Hello World!", 16);  // 12 -> 16
  base64test("Hello World", 15);   // 11 -> 15
  base64test("Hello Worl", 14);    // 10 -> 14
  base64test("Hello Wor", 12);     // 9 -> 12
  base64test("Hello Wo", 11);      // 8 -> 11
}
