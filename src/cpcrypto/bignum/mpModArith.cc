...
bool mpShiftUpWords(bnum& bnN, int r, bnum& bnOut) {
  ...
  if (bnOut.mpSize() < (lN + r)) return false;

  for (j = 0; j < lN; j++) bnOut.m_pValue[j + r] = bnN.m_pValue[j];
  for (j = 0; j < r; j++) bnOut.m_pValue[j] = 0ULL;
  return true;
}
...
bool mpShiftDownWords(bnum& bnN, int r, bnum& bnOut) {
  ...
  for (j = lN; j < bnOut.mpSize(); j++) bnOut.m_pValue[j] = 0ULL;
  return true;
}
...
inline bool mpModPowerofTwo(bnum& bnN, int r)
  ...
  for (j = r; j < bnN.mpSize(); j++) bnN.m_pValue[j] = 0ULL;
  return true;
}
...
bool mpMakeMont(bnum& bnN, bnum& bnM, int r, bnum& bnmontN)
  ...
  try {
    ...
    if (!mpShiftUpWords(bnN, r, bnU)) throw("mpShiftUpWords error");
...
bool mpMontStep(bnum& bnmontA, bnum& bnmontB, bnum& bnM, bnum& bnMPrime, int r,
                bnum& bnmontC)
...
bool mpMontInit(int r, bnum& bnM, bnum& bnMPrime, bnum& bnRmodM,
  ...
  try {
    ...
    if (!mpMod(bnR, bnM, bnRmodM)) throw("cant get R mod M");
...
bool mpMontModExp(bnum& bnBase, bnum& bnExp, bnum& bnM, bnum& bnOut, int r,
  ...
  try {
    ...
    if (!mpMontStep(bnBase, bnRsqmodM, bnM, bnMPrime, r,
                    *rgbnmontBasePower[evenBP])) {
    ...
    for (j = 2; j <= leadBit; j++) {
      if (!mpMontStep(*rgbnmontBasePower[evenBP], *rgbnmontBasePower[evenBP],
                      bnM, bnMPrime, r, *rgbnmontBasePower[evenBP ^ 1])) {
  ...
  if (rgbnmontBasePower[0] != NULL) delete rgbnmontBasePower[0];
  if (rgbnmontBasePower[1] != NULL) delete rgbnmontBasePower[1];
  if (rgbnmontA[0] != NULL) delete rgbnmontA[0];
  if (rgbnmontA[1] != NULL) delete rgbnmontA[1];
...
bool mpRSACalculateFastRSAParameters(bnum& bnE, bnum& bnP, bnum& bnQ,
                                     bnum& bnPM1, bnum& bnDP, bnum& bnQM1,
                                     bnum& bnDQ)
  ...
  try {
    ...
    if (mpUSub(bnP, g_bnOne, bnPM1) != 0ULL) throw("Can't compute PM1");

    if (mpUSub(bnQ, g_bnOne, bnQM1) != 0ULL) throw("Can't compute QM1");
    ...
    while (bnDP.mpSign()) mpAddTo(bnDP, bnPM1);
...
bool mpRSAMontDEC(bnum& bnMsg, bnum& bnP, bnum& bnPM1, bnum& bnDP, bnum& bnQ,
                  bnum& bnQM1, bnum& bnDQ, bnum& bnM, bnum& bnR, int r,
                  bnum& bnPPrime, bnum& bnRmodP, bnum& bnRsqmodP,
  ...
  try {
    ...
    if (!mpSlidingModExp(bnMsg, bnDP, bnP, bnT1, lM / 2, bnPPrime, bnRmodP,
                         bnRsqmodP))
      throw("mpMontModExp fails 1");
    if (!mpSlidingModExp(bnMsg, bnDQ, bnQ, bnT2, lM / 2, bnQPrime, bnRmodQ,
                         bnRsqmodQ))
      throw("mpMontModExp fails 2");
#else
    if (!mpMontModExp(bnMsg, bnDP, bnP, bnT1, lM / 2, bnPPrime, bnRmodP,
                      bnRsqmodP))
      throw("mpMontModExp fails");
    if (!mpMontModExp(bnMsg, bnDQ, bnQ, bnT2, lM / 2, bnQPrime, bnRmodQ,
                      bnRsqmodQ))
      throw("mpMontModExp fails");
#endif
...
bool mpRSADEC(bnum& bnMsg, bnum& bnP, bnum& bnPM1, bnum& bnDP, bnum& bnQ,
              bnum& bnQM1, bnum& bnDQ, bnum& bnM, bnum& bnR)
    //  Fast RSA Decrypt using Chinese remainer theorem
    //  Call mpCRT(Msg^d(p),p,Msg^d(q),q, R)
    //  Return R
    {
  ...
  try {
    ...
    if (!mpModExp(bnMsg, bnDP, bnP, bnT1)) throw("mpModExp fails");
    if (!mpModExp(bnMsg, bnDQ, bnQ, bnT2)) throw("mpModExp fails");
    if (!mpCRT(bnT1, bnP, bnT2, bnQ, bnR)) throw("mpCRT fails");
  }
  catch (const char * sz) {
...
bool mpSlidingModExp(bnum& bnBase, bnum& bnExp, bnum& bnM, bnum& bnR, int r,
                     bnum& bnMPrime, bnum& bnRmodM, bnum& bnRsqmodM) {
  int j, v;
  bool fRet = true;
  int lM = mpWordsinNum(bnM.mpSize(), bnM.m_pValue);
...
// for(i=0; i<((1<<k)-1); i++)  g(2i+1)= g(2i-1)*g2;
    ...
    for (k = 0; k < 7; k++) {
      if (!mpMontStep(*rgTable[2], *rgTable[2 * k + 1], bnM, bnMPrime, r,
                      *rgTable[2 * k + 3])) {
    ...
    for (j = leadBit; j > 0; j--) {
      ...
      //         bnA= (bnA**(2^l)*g(n)
      if (!IsBitPositionNonZero(bnExp, j)) {
        if (!mpMontStep(*rgbnmontA[even], *rgbnmontA[even], bnM, bnMPrime, r,
                        *rgbnmontA[even ^ 1])) {
          throw("MontStep error 2");
        }
        ...
        if (!mpMontStep(*rgbnmontA[even], *rgTable[v], bnM, bnMPrime, r,
                        *rgbnmontA[even ^ 1])) {
          throw("MontStep error 4");
        }
#ifdef MPSLIDINGTEST
