...
void bidTester::getParams(const TiXmlNode* parent, const string& parentPath, bidTestParams& params) {
    ...
    while((child = parent->IterateChildren(child))) {
        ...
        if (name.compare("Seller") == 0) {
            ...
            const TiXmlNode* clientNode = child->FirstChild();
            if (NULL == clientNode) {
                throw "Seller nodes must have a child Client node\n";
            }

            getClientParams(clientNode, parentPath, params.seller);
            ...
            while((bidderChild = child->IterateChildren(bidderChild))) {
                ...
                if (!bidderNodeName.compare("Bidder") == 0) {
                ...
                while ((bidderChildNode = bidderChild->IterateChildren(bidderChildNode))) {
                    ...
                    if (bidderChildName.compare("Client") == 0) {
                        ...
                        foundClient = true;
                        getClientParams(bidderChildNode, parentPath, bcp.first);
                    } else if (bidderChildName.compare("Bid") == 0) {
                        // handle the Bid case
                        if (foundBid) {
                        ...
                        const TiXmlElement* bidderChildElt = bidderChildNode->ToElement();
                        int bidValue = 0;
                        ...
                        const bidTestParams& params)
{
    ...
    try {
            ...
            // now have the sellerClient connect to get its keys
            //sellerClient sellerClient;
            //safeChannel sellerChannel;
            
            //  
//            result = sellerClient.establishConnection(sellerChannel,
//                                    params.seller.keyFile.c_str(),
//                                    params.seller.certFile.c_str(),
//                                    directory,
//                                    m_serverAddress.c_str(),
//                                    m_serverPort);
//            if (!result) {
//                if (m_printToStdout) printf("[FAILED]\n");
//                fprintf(g_logFile, "Seller failed to connect to bidServer\n");
//                return false;
//            }
...
//                sellerClient.getResults(); 
//                string winner = sellerClient.getWinner();
//                pass = result && (winner.compare(params.expectedWinner) == 0);
//
//                if (params.timed) {
...
//                sellerTestTimer.print(g_logFile);
            ...
            fprintf(g_logFile, "Got an unexpected error in bidTester: %s\n", err);
        }
