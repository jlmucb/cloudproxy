...
static LKUP_STATUS strncmp_local(char *string1, char *string2, int len2);
...
int vt100_getpos(unsigned *line, unsigned *col)
    ...
    do  {
        ...
        if ((key = vt100_getch_raw()) != ASCII_ESCAPE)
            break;

        if ((key = vt100_getch_raw()) != '[')
            break;

        if ((key = read_dec_number(line)) != ';')
            break;

        if ((key = read_dec_number(col)) != 'R')
            break;
