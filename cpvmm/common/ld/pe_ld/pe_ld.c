...
perform_base_relocations(
                        PE_IMAGE_TYPE           image_type,
                        IMAGE_NT_HEADERS32*     p_nt_header_32,
                        char*                   image_base_address )
{
    ...
    if ((preferred_load_addr.fields32.high32 == 0) &&
        (preferred_load_addr.fields32.low32 == 
    ...
    if (!base_relocs_data_entry.VirtualAddress ||
        !base_relocs_data_entry.Size)
    {
    ...
    while (base_relocs_block != base_relocs_block_limit)
    {
        ...
        number_of_relocs_in_group =
            (current_base_relocs_block->SizeOfBlock - IMAGE_SIZEOF_BASE_RELOCATION)
            / sizeof(WORD);

        reloc_group_arr = (WORD*)
            ((char*)current_base_relocs_block + IMAGE_SIZEOF_BASE_RELOCATION);
        ...
        for (reloc_index = 0; reloc_index < number_of_relocs_in_group; ++reloc_index)
        {
            ...
            else if (reloc_type == IMAGE_REL_BASED_HIGHLOW)
            {
                ...
                PE_TESTING(printf("Updating 32 bit value at 0x%x from 0x%x to 0x%x\n",
                                    curr_page + reloc_offset,
                                    *((INT32*)(curr_page + reloc_offset)),
...
void
copy_image_sections(
                        const char*           file_mapped_into_memory,
                        ...
                        UINT32                allocated_size )
{
    ...
    for (section_index = 0; section_index < number_of_sections; ++section_index)
    {
        ...
        if (p_section_header->PointerToRawData)
        {
            ...
            amount_to_copy = (p_section_header->SizeOfRawData < p_section_header->Misc.VirtualSize) ?
        ...
        if (p_section_header->SizeOfRawData < p_section_header->Misc.VirtualSize)
        {
            PE_TESTING(printf("Zeroing uninit data for section %s 0x%x. Size 0x%x\n",
                                p_section_header->Name,
                                loaded_buffer + p_section_header->SizeOfRawData,
                                p_section_header->Misc.VirtualSize - p_section_header->SizeOfRawData ));

            vmm_memset( loaded_buffer + p_section_header->SizeOfRawData,
                        0,
                        p_section_header->Misc.VirtualSize - p_section_header->SizeOfRawData );
        }
    ...
    )
{
    ...
    if ((import_table_data_entry.Size != 0) ||
        (import_table_data_entry.VirtualAddress != 0))
    {
    ...
    )
{
    ...
    if (get_PE_image_info( file_mapped_into_memory, &image_info)
        != GET_PE_IMAGE_INFO_OK)
    {
        // wrong image
        return FALSE;
    }
    ...
    image_type = (image_info.machine_type == PE_IMAGE_MACHINE_X86) ?
                    PE_IMAGE32 : PE_IMAGE64;

    p_nt_header_32 = (IMAGE_NT_HEADERS32*)
         ((char*)file_mapped_into_memory +
                    ((IMAGE_DOS_HEADER*)file_mapped_into_memory)->e_lfanew);
    ...
    copy_image_sections( (char*)file_mapped_into_memory,
                         section_table,
                         p_nt_header_32->FileHeader.NumberOfSections,
                         (char*)image_base_address,
                         allocated_size );

    // 2. now perform base relocation
    if (! perform_base_relocations( image_type,
                                    p_nt_header_32,
                                    (char*)image_base_address ))
    {
        return FALSE;
