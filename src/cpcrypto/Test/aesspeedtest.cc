...
u8 tkey[16] = {0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15,
               0x88, 0x09, 0xcf, 0x4f, 0x3c};

u8 tplain[16] = {0x6b, 0xc1, 0xbe, 0xe2, 0x2e, 0x40, 0x9f, 0x96, 0xe9, 0x3d,
...
int main(int an, char** av) {
  ...
  if (mode == CBCMODE) printf("CBC Mode test, %d blocks\n", numBlocks);
  if (mode == GCMMODE) printf("GCM Mode test, %d blocks\n", numBlocks);
  if (mode == ECBMODE) printf("ECB Mode test, %d blocks\n", numBlocks);

  initCryptoRand();
  if (mode == CBCMODE) {
    if (!EncryptCBCTest(numBlocks)) printf("Test failed\n");
  }
  if (mode == ECBMODE) {
    if (fUseNI) {
      if (!EncryptECBNITest(numBlocks)) printf("Test failed\n");
    } else {
      if (!EncryptECBTest(numBlocks)) printf("Test failed\n");
    }
  }
  closeCryptoRand();
