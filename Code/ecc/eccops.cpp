...
void ECPoint::printMe()
{
    ...
    if(m_myCurve==NULL)
        fprintf(g_logFile, "No curve set\n");

    fprintf(g_logFile, "X: "); printNum(*m_bnX); fprintf(g_logFile, "\n");
    fprintf(g_logFile, "Y: "); printNum(*m_bnY); fprintf(g_logFile, "\n");
    fprintf(g_logFile, "Z: "); printNum(*m_bnZ); fprintf(g_logFile, "\n");
    return;
}
#endif
...
void ECurve::printMe()
{
    fprintf(g_logFile, "\nCurve\n");
    if(m_bnM!=NULL) {
        fprintf(g_logFile, "M: "); printNum(*m_bnM); fprintf(g_logFile, "\n");
    }
    else
        fprintf(g_logFile, "M is not set\n");
    if(m_bnA!=NULL) {
        fprintf(g_logFile, "A: "); printNum(*m_bnA); fprintf(g_logFile, "\n");
    }
    else
        fprintf(g_logFile, "A is not set\n");
    if(m_bnB!=NULL) {
        fprintf(g_logFile, "B: "); printNum(*m_bnB); fprintf(g_logFile, "\n");
    }
    else
        fprintf(g_logFile, "B is not set\n");
    if(m_bnDisc!=NULL) {
        fprintf(g_logFile, "Disc: "); printNum(*m_bnDisc); fprintf(g_logFile, "\n");
    }
    else
        fprintf(g_logFile, "Disc is not set\n");
    if(m_bnGx!=NULL) {
        fprintf(g_logFile, "Gx: "); printNum(*m_bnGx); fprintf(g_logFile, "\n");
    }
    else
        fprintf(g_logFile, "Gx is not set\n");
    if(m_bnGy!=NULL) {
        fprintf(g_logFile, "Gy: "); printNum(*m_bnGy); fprintf(g_logFile, "\n");
    }
    else
...
bool copynumwithalloc(bnum** bn1, bnum& bn2)
{
    ...
    if(*bn1==NULL)
        return false;
    int i;
    for(i=0; i<bn2.mpSize(); i++)
        (*bn1)->m_pValue[i]= bn2.m_pValue[i];
    for(; i<(*bn1)->mpSize();i++)
        (*bn1)->m_pValue[i]= 0ull;
    if((*bn1)->mpSign()!=bn2.mpSign())
        (*bn1)->mpNegate();
    return true;
}
...
bool ECPoint::copyPoint(ECPoint& P)
{
    ...
    if(!copynumwithalloc(&m_bnX, *P.m_bnX))
        return false;
    if(!copynumwithalloc(&m_bnY, *P.m_bnY))
        return false;
    if(!copynumwithalloc(&m_bnZ, *P.m_bnZ))
        return false;
    return true;
}
...
// −(4a^3+27b^2)
{
    if(m_bnDisc!=NULL)
        return m_bnDisc;
    if(m_bnM==NULL || m_bnA==NULL || m_bnB==NULL)
        return NULL;
...
void ECKey::printMe()
{
    ...
    if(m_secret!=NULL) {
        fprintf(g_logFile, "secret: "); printNum(*m_secret); fprintf(g_logFile, "\n");
    }
    else
...
bool ECKey::makePrivateKey()
{
    ...
    if(!getCryptoRandom(NUMBITSINU64*m_myCurve->m_bnM->mpSize(), (byte*)m_secret->m_pValue))
        return false;
...
bool ecInv(ECPoint& P, ECPoint& R)
{
    ...
    R.m_bnX= P.m_bnX;
    R.m_bnZ= P.m_bnZ;
    mpModSub(*(P.m_myCurve->m_bnM), *(P.m_bnY), *(P.m_myCurve->m_bnM), *(R.m_bnY));
