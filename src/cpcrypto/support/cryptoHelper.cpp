...
bool RSASign(RSAKey& key, int hashType, byte* hash, int* psizeout, byte* out) {
...
#endif
  if (!RSADecrypt(key, key.m_iByteSizeM, padded, psizeout, out)) return false;
...
RSAKey* RSAKeyfromKeyInfoNode(TiXmlNode* pNode) {
  ...
  try {
    ...
    if (szDoc == NULL) throw("Cant canonicalize keyinfo\n");

    if (!pKey->ParsefromString(szDoc)) throw("Cant parse KeyInfo\n");
...
char* RSAKeyInfofromKey(RSAKey& key) { return key.SerializePublictoString(); }
...
bool sameRSAKey(RSAKey* pKey1, RSAKey* pKey2) {
  if (pKey1->m_iByteSizeM != pKey2->m_iByteSizeM) return false;
  if (memcmp(pKey1->m_rgbM, pKey2->m_rgbM, pKey1->m_iByteSizeM) != 0)
    return false;
  if (pKey1->m_iByteSizeE != pKey2->m_iByteSizeE) return false;
...
bool timeInfofromstring(const char* szTime, struct tm& thetime) {
  sscanf(szTime, "%04d-%02d-%02dZ%02d:%02d.%02d", &thetime.tm_year,
         &thetime.tm_mon, &thetime.tm_mday, &thetime.tm_hour, &thetime.tm_min,
         &thetime.tm_sec);
...
bool checktimeinInterval(tm& time, tm& begin, tm& end)
    // is time less than or equal to endtime and greater than or equal to begin
    // time
    {
...
bool XMLenclosingtypefromelements(const char* tag, int numAttr,
                                  const char** attrName,
                                  const char** attrValues, int numElts,
                                  const char** elts, int* psize, char* buf) {
  int size;
  int n = 0;
  int i;
...
bool VerifyRSASha1SignaturefromSignedInfoandKey(RSAKey& key, char* szsignedInfo,
                                                char* szSigValue) {
  Sha1 oHash;
...
char* XMLRSASha256SignaturefromSignedInfoNodeandKey(RSAKey& key,
  ...
  char* szSig = XMLRSASha256SignaturefromSignedInfoandKey(key, szsignedInfo);
...
char* constructXMLRSASha256SignaturefromSignedInfoandKey(
    RSAKey& key, const char* id, const char* szsignedInfo) {
  ...
  fRet = XMLenclosingtypefromelements("ds:Signature", 2, attrName, attrValue, 3,
                                      elts, &size, szSignature);

  if (szKeyInfo != NULL) {
    free(szKeyInfo);
    szKeyInfo = NULL;
  }
  if (fRet) return strdup(szSignature);
  return NULL;
}
