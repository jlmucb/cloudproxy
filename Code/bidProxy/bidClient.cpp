...
bool bidClient::protocolNego(int fd, safeChannel& fc, const char* keyFile, const char* certFile)
{
    ...
    request[0]= '\0';
#ifdef TEST
    fprintf(g_logFile, "bidClient: protocol negotiation\n");
    fflush(g_logFile);
#endif
    ...
    try {
        ...
        if(!clientNegoMessage2(request, MAXREQUESTSIZE, szEncPreMasterSecret,
            ...
            throw  "bidClient: Can't format negotiation message 2";
        if(!m_oKeys.updateMessageHash(strlen(request), (byte*) request))
            throw  "bidClient::protocolNego: Can't update message hash";
        if((n=sendPacket(fd, (byte*)request, strlen(request)+1, CHANNEL_NEGO, 0, 1))<0)
            throw  "bidClient: Can't send packet 2";

        if(!m_oKeys.clientcomputeMessageHash()) 
            throw "bidClient::protocolNego: client cant compute message hash";
        if(!m_oKeys.clientsignMessageHash()) 
            throw "bidClient::protocolNego: client cant sign message hash";
        if(!clientNegoMessage3(request, MAXREQUESTSIZE, m_oKeys.m_szbase64SignedMessageHash))
            throw  "bidClient: Can't format negotiation message 3";
        if(!m_oKeys.updateMessageHash(strlen(request), (byte*) request))
            throw  "bidClient::protocolNego: Can't update message hash";
        if((n=sendPacket(fd, (byte*)request, strlen(request)+1, CHANNEL_NEGO, 0, 1))<0)
            throw  "bidClient: Can't send packet 2";
        ...
        if(!initSafeChannel(fc))
            throw  "bidClient: Can't init safe channel";
#ifdef TEST
        fprintf(g_logFile, "bidClient: initsafeChannel succeeded\n");
#endif

        // Assume CBC
        if((n=getPacket(fd, fc.lastgetBlock, AES128BYTEBLOCKSIZE, &type, &multi, &final))<0)
            throw "bidClient: Cant get IV\n";
        fc.fgetIVReceived= true;
        if((n=sendPacket(fd, fc.lastsendBlock, AES128BYTEBLOCKSIZE, CHANNEL_NEGO_IV, 0, 1))<0)
            throw "bidClient: Cant send IV\n";
        fc.fsendIVSent= true;
#ifdef  TEST
        fprintf(g_logFile, "bidClient::protocolNego: Encrypted mode on\n");
        ...
        if((n=fc.safegetPacket((byte*)request, MAXREQUESTSIZE, &type, &multi, &final))<0)
            throw  "bidClient: Can't get server packet 2";
        if(!m_oKeys.servercomputeMessageHash())
            throw  "bidClient::protocolNego: Can't compute server hash";
        if(!getDatafromServerMessage2(n, request, m_oKeys))
            throw  "bidClient::protocolNego: Can't decode server message 2";

        // do hashes match?
#ifdef TEST
        fprintf(g_logFile, "bidClient::protocolNego: server hases\n");
    ...
    fflush(g_logFile);
        ...
        if(!m_oKeys.m_fServerMessageHashValid || 
           ...
           memcmp(m_oKeys.m_rgServerMessageHash, m_oKeys.m_rgDecodedServerMessageHash, 
            ...
            throw  "bidClient::protocolNego: server hash does not match";

        // Phase 4, send
        if(!m_oKeys.getPrincipalPrivateKeysFromFile(keyFile))  
            throw  "bidClient: Cant principal private keys from file";
#ifdef TEST
        fprintf(g_logFile, "bidClient: got principal keys\n");
#endif
        if(!m_oKeys.getPrincipalCertsFromFile(certFile))
            throw  "bidClient: Cant get principal private keys from file";
        if(!m_oKeys.initializePrincipalPrivateKeys())
            throw  "bidClient: Cant initialize principal private keys";
#ifdef TEST
        fprintf(g_logFile, "bidClient: got principal private keys\n");
#endif
        if(!m_oKeys.initializePrincipalCerts())
            throw  "bidClient: Cant initialize principal certs\n";
        ...
        if(strcmp(m_oKeys.m_szChallengeSignAlg, "TLS_RSA2048_WITH_AES128_CBC_SHA256")!=0 &&
           strcmp(m_oKeys.m_szChallengeSignAlg, "TLS_RSA1024_WITH_AES128_CBC_SHA256")!=0)
            throw  "bidClient: Unsupported challenge algorithm\n";
...
#ifdef TEST
        fprintf(g_logFile, "bidClient: challenges encoded\n");
#endif
        if(szSignedChallenges==NULL)
            throw  "bidClient: Can't sign principal challenges";
        ...
        if((n=fc.safesendPacket((byte*)request, strlen(request)+1, CHANNEL_NEGO, 0, 1))<0)
            throw  "bidClient: Can't send packet 3";

        // Phase 3, receive
        if((n=fc.safegetPacket((byte*)request, MAXREQUESTSIZE, &type, &multi, &final))<0)
            throw  "bidClient: Can't get packet 3";

        if(!getDatafromServerMessage3(n, request, m_oKeys))
            throw  "bidClient: Can't decode client message 3";

        m_oKeys.validateChannelData(true);
#ifdef TEST
        fprintf(g_logFile, "bidClient: channel data validated\n");
#endif
#if 0
        // register principals
        if(m_oKeys.m_pserverCert!=NULL) {
            if(registerPrincipalfromCert(m_oKeys.m_pserverCert)==NULL)
                throw "bidClient: Can't register server principal\n";
        }
#ifdef TEST
        fprintf(g_logFile, "bidClient: server principal registered\n");
#endif

        if(registerPrincipalfromCert(m_oKeys.m_pclientCert)==NULL)
            throw "bidClient: Can't register client principal\n";
#endif
#ifdef TEST
        fprintf(g_logFile, "bidClient: server principal registered\n");
#endif
...
#ifdef TEST
        fprintf(g_logFile, "bidClient: protocol nego succesfully completed\n");
        fflush(g_logFile);
#endif
    }
    catch(const char* szError) {
        fprintf(g_logFile, "bidClient: Protocol Nego error: %s\n", szError);
        fRet= false;
        fflush(g_logFile);
    }
