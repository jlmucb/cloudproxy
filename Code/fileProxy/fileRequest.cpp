...
bool  fileRequest::getDatafromDoc(const char* szRequest)
{
    ...
    if(!doc.Parse(szRequest)) {
        fprintf(g_logFile, "fileRequest::getDatafromDoc: Cant parse request\n");
        return false;
    }

    pRootElement= doc.RootElement();
    if(strcmp(pRootElement->Value(),"Request")!=0) {
        fprintf(g_logFile, "fileRequest::getDatafromDoc: Should be request\n");
        return false;
    }
...
#ifdef TEST1
    fprintf(g_logFile, "fileRequest getdata\n");
    printMe();
    fflush(g_logFile);
#endif
    return true;
...
bool  fileResponse::getDatafromDoc(char* szResponse)
{
    TiXmlDocument   doc;
    TiXmlElement*   pRootElement= NULL;
    TiXmlNode*      pNode= NULL;
    ...
    if(!doc.Parse(szResponse)) {
        fprintf(g_logFile, "fileResponse::getDatafromDoc: cant parse response\n");
        return false;
    }

    pRootElement= doc.RootElement();
    if(strcmp(pRootElement->Value(),"Response")!=0) {
        fprintf(g_logFile, "fileResponse::getDatafromDoc: Should be response\n");
        return false;
    }
...
#ifdef TEST1
    fprintf(g_logFile, "fileResponse getdata\n");
    printMe();
    fflush(g_logFile);
#endif
    return true;
