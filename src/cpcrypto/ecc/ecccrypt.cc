 ...
 * ECC Decrypt: Receive (L, K) calculate K − (secret)L = M and map it back to
* the 
 *      integer message. 
* 
...
bool ecSign(ECKey& K, bnum& bnH, bnum& bnR, bnum& bnS) {
  ...
  if (!getCryptoRandom(NUMBITSINU64 * K.m_myCurve->m_bnM->mpSize(),
                       (byte*)bnK.m_pValue))
    return false;
  bnK.m_pValue[bnK.mpSize() - 1] &= 0x7fffffffffffffff;
  if (!ecMult(*K.m_G, bnK, Q)) return false;

  // r= Qx (mod orderG)
  mpMod(*Q.m_bnX, *K.m_myCurve->m_bnorderG, bnR);
  // s= k^(-1)(h+r(secret)) (mod orderG)
  if (!mpModInv(bnK, *K.m_myCurve->m_bnorderG, bnKInv)) return false;
...
bool ecVerify(ECKey& K, bnum& bnH, bnum& bnR, bnum& bnS) {
  ...
  if (!mpModInv(bnS, *K.m_myCurve->m_bnorderG, bnSInv)) return false;
  mpModMult(bnSInv, bnH, *K.m_myCurve->m_bnorderG, bnU);
  mpModMult(bnSInv, bnR, *K.m_myCurve->m_bnorderG, bnV);

  // R= uG+vQ.  Valid if Rx=r.
  if (!ecMult(*K.m_G, bnU, A)) return false;
  if (!ecMult(*K.m_Public, bnV, B)) return false;
  if (!ecAdd(A, B, R)) return false;
  return mpCompare(bnR, *R.m_bnX) == 0;
}

#ifdef TEST
bool ecSignwithgivennonce(ECKey& K, bnum& bnH, bnum& bnK, bnum& bnR,
                          bnum& bnS) {
  bnum bnKInv(K.m_myCurve->m_bnM->mpSize());
  bnum bnT(K.m_myCurve->m_bnM->mpSize() + 1);
  bnum bnTA(K.m_myCurve->m_bnM->mpSize() + 1);
...
bool ecEncrypt(ECKey& K, bnum& bnX, ECPoint& R1, ECPoint& R2) {
  ...
  if (!getCryptoRandom(NUMBITSINU64 * K.m_myCurve->m_bnM->mpSize(),
    ...
    return false;
  bnK.m_pValue[bnK.mpSize() - 1] &= 0x7fffffffffffffff;
  if (!ecMult(*K.m_G, bnK, R1)) return false;
  if (!ecMult(*K.m_Public, bnK, bnT1)) return false;
  if (!ecAdd(bnT1, M, R2)) return false;
  return true;
}
...
bool ecDecrypt(ECKey& K, ECPoint& P1, ECPoint& P2, bnum& bnX) {
  ...
  if (!ecMult(P1, *K.m_secret, bnT1)) return false;
  if (!ecSub(P2, bnT1, M)) {
    return false;
  }
#ifdef TEST
