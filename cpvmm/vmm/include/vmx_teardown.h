...
typedef struct VMM_TEARDOWN_GUEST_STATES_S{
    ...
    // general purpose registers
        ...
        // This field is supported only on logical processors that 
        // support the 1-setting of the "load IA32_PAT" VM-entry control.
        UINT64 IA32_MSR_PAT_REG;
        
        //This field is supported only on logical processors that 
        // support the 1-setting of the "load IA32_EFER" VM-entry control.
...
BOOLEAN vmexit_vmm_teardown(GUEST_CPU_HANDLE gcpu, 
            VMM_TEARDOWN_PARAMS *vmm_teardown_params);
BOOLEAN vmam_add_to_host_page_table(IN GUEST_CPU_HANDLE gcpu, 
            IN UINT64 start_gva, IN UINT64 num_pages);
