...
bool MontEncryptTest(RSAKey* pKey, int numBlocks, bool fEncrypt)
{
    ...
    *puIn= 237ULL;
    ...
    if(numBlocks==1) {
        ...
        if(!mpSlidingModExp(bnEncrypted, *(pKey->m_pbnD), *(pKey->m_pbnM), bnDecrypted,
                         r, bnMPrime, bnRmodM, bnRsqmodM)) {
            printf("Can't decrypt\n");
            return false;
        }
#endif
        printf("\nIn       : "); printNum(bnIn); printf("\n");
...
#ifndef SLIDINGWINDOW
            if(!mpMontModExp(bnIn, *(pKey->m_pbnE), *(pKey->m_pbnM), bnEncrypted,
                             r, bnMPrime, bnRmodM, bnRsqmodM)) {
                printf("Can't encrypt\n");
                return false;
            }
        ...
        for(i=0; i<numBlocks; i++) {
#ifndef SLIDINGWINDOW
            if(!mpMontModExp(bnIn, *(pKey->m_pbnD), *(pKey->m_pbnM), bnDecrypted,
                             r, bnMPrime, bnRmodM, bnRsqmodM)) {
                printf("Can't decrypt\n");
                return false;
            }
    ...
    if(fEncrypt)
        printf("RSA Mont Encrypt: %10.2f seconds, %10d operations, %10.1f ops/second\n", 
                elapsedseconds, numBlocks, opspersecond);
    else
        printf("RSA Mont Decrypt: %10.2f seconds, %10d operations, %10.1f ops/second\n", 
                elapsedseconds, numBlocks, opspersecond);
    return true;
}
...
bool EncryptTest(RSAKey* pKey, int numBlocks, bool fFast= false, bool fEncrypt=true)
{
    ...
    *puIn= 237ULL;
    ...
    if(!mpMontInit(r, *(pKey->m_pbnP), bnPPrime, bnRmodP, bnRsqmodP)) {
        printf("RSA Montgomery Encrypt: can't initialize Montgomery components(P)\n");
        return false;
    }
    r= mpWordsinNum(pKey->m_pbnQ->mpSize(), pKey->m_pbnQ->m_pValue);
    if(!mpMontInit(r, *(pKey->m_pbnQ), bnQPrime, bnRmodQ, bnRsqmodQ)) {
        printf("RSA Montgomery Encrypt: can't initialize Montgomery components(Q)\n");
        return false;
    }
#endif
#endif
...
printf("fastDecrypt test\n");
        for(i=0; i<numBlocks; i++) {
            if(!mpRSAMontDEC(bnIn, *(pKey->m_pbnP), bnPM1, bnDP, *(pKey->m_pbnQ), 
                             bnQM1, bnDQ, *(pKey->m_pbnM), bnDecrypted, r,
                             bnPPrime, bnRmodP, bnRsqmodP,
                             bnQPrime, bnRmodQ, bnRsqmodQ)) {
                printf("Can't decrypt\n");
                return false;
            }
            (*puIn)++;
...
bool RSASanityCheck(RSAKey* key, int file, bool fPrint, bool fFast)
{
    ...
    for(;;) {
        ...
        if(fFast) {
#ifdef MONTGOMERYENABLED
            if(!mpRSAMontDEC(bnEncrypted, *(key->m_pbnP), bnPM1, bnDP, *(key->m_pbnQ), 
                           bnQM1, bnDQ, *(key->m_pbnM), bnDecrypted,r,
                           bnPPrime, bnRmodP, bnRsqmodP,
                           bnQPrime, bnRmodQ, bnRsqmodQ)) {
                printf("Can't decrypt\n");
                fFailed= true;
    ...
    ops= (double)numBlocks;
    opspersecond= ops/elapsedseconds;
    printf("RSA Enc/Decrypt: %10.2f seconds, %10d operations, %10.1f ops/second\n", 
        elapsedseconds, numBlocks, opspersecond);
    return fRet;
}
...
bool Addtest(int num)
{
    ...
    for(i=0;i<16;i++)
        bnIn1.m_pValue[i]= (i+1)*23;
    for(i=0;i<16;i++)
        bnIn2.m_pValue[i]= (i+1)*31;
    time(&start);

    for(i=0;i<unum;i++)
        mpUAdd(bnIn1, bnIn2, bnIn3);
...
bool Divtest(int num)
{
    ...
    for(i=0;i<unum;i++)
        mpUDiv(bnIn1, bnIn2, bnIn3, bnIn4);
...
bool Exptest(int num)
{
    ...
    for(i=0;i<15;i++)
        bnIn1.m_pValue[i]= (i+1)*23;
    for(i=0;i<15;i++)
        bnIn2.m_pValue[i]= (i+1)*17;
    for(i=0;i<16;i++)
        bnIn3.m_pValue[i]= (i+1)*31;

    time(&start);

    for(i=0;i<unum;i++)
        mpModExp(bnIn1, bnIn3, bnIn2, bnIn4);
...
bool GCDtest(int num)
{
    ...
    for(i=0;i<unum;i++)
        mpExtendedGCD(bnIn1, bnIn2, bnIn3, bnIn4, bnIn5);
...
int main(int an, char** av)
{
    ...
    if(!(fSanityOnly || fAdd || fSub || fMultiply || fPureMultiply ||
                        fDiv || fExp || fMod || fGCD || fEncrypt || 
                        fDecrypt || fMontEncrypt || fMontDecrypt ||
                        fSlowSquare || fFastSquare || fBinaryGCD)) {
    ...
    if(fPureMultiply) {
        if(Puremult(numBlocks))
            return 0;
        return 1;
    }
    else if(fAdd) {
        if(Addtest(numBlocks))
            return 0;
        return 1;
    }
    else if(fMultiply) {
        if(Multiplytest(numBlocks))
            return 0;
        return 1;
    }
    else if(fSlowSquare) {
        if(SlowSquaretest(numBlocks))
            return 0;
        return 1;
    }
    else if(fFastSquare) {
        if(FastSquaretest(numBlocks))
            return 0;
        return 1;
    }
    else if(fSub) {
        if(Subtest(numBlocks))
            return 0;
        return 1;
    }
    else if(fDiv) {
        if(Divtest(numBlocks))
            return 0;
        return 1;
    }
    else if(fExp) {
        if(Exptest(numBlocks))
            return 0;
        return 1;
    }
    else if(fMod) {
        if(Modtest(numBlocks))
            return 0;
        return 1;
    }
    else if(fGCD) {
        if(GCDtest(numBlocks))
            return 0;
        return 1;
    }
    else if(fBinaryGCD) {
        if(BinaryGCDtest(numBlocks))
            return 0;
        return 1;
    }
