...
#ifdef CRYPTOTEST
    PrintBytes((char*)"decoded hash\n", &rgSig[n], hashLen);
    PrintBytes((char*)"computed hash\n", rgHash, hashLen);
#endif
        return false;
    }
    n+= hdrLen;
...
bool gcm::initDec(u32 alg, int keysize, byte* key, int cipherLen, int authLen, int tagLen)
{
    ...
    computePlainLen();
#ifdef CRYPTOTEST5
    PrintBytes((char*)"H", m_rguH, m_iBlockSize);
#endif
    return true;
}
...
// should finalizeCipher first
{
    ...
    *((u32*)rguLens)= reverseInt(NBITSINBYTE*m_iNumAuthBytes);
    *((u32*)&rguLens[12])= reverseInt(NBITSINBYTE*m_iNumPlainBytes);
...
#ifdef CRYPTOTEST
    PrintBytes((char*)"gcm::nextPlainBlockIn, in", puIn, m_iBlockSize);
#endif
    incY();
...
bool gcm::nextCipherBlockIn(byte* puIn, byte* puOut)
{
    ...
    inlineXor(puOut, oldX, puIn, m_iBlockSize);
#ifdef CRYPTOTEST
    PrintBytes((char*)"gcm::nextCipherBlockIn, out", puOut, m_iBlockSize);
#endif
    return true;
}
...
{
#ifdef CRYPTOTEST5
    PrintBytes((char*)"gcm::firstCipherBlockIn, in", puIn, m_iBlockSize);
#endif
...
{
#ifdef CRYPTOTEST
    PrintBytes((char*)"gcm::firstCipherBlockout", m_rguFirstBlock, m_iBlockSize);
#endif
...
{
#ifdef CRYPTOTEST5
    PrintBytes((char*)"gcm::validateTag, computed", m_rgTag, m_iBlockSize);
    PrintBytes((char*)"gcm::validateTag, sent", m_rgsentTag, m_iBlockSize);
#endif
...
int gcm::lastPlainBlockIn(int size, byte* puIn, byte* puOut)
{
    ...
    PrintBytes((char*)"lastPlainBlockIn", puIn, m_iBlockSize);
#endif
...
//  0 < size <= m_iBlockSize+tagsize
{
    ...
    PrintBytes((char*)"gcm::lastCipherBlockIn, in", puIn, m_iBlockSize);
#endif
...
{
    m_iNumPlainBytes= m_iNumCipherBytes-3*m_iBlockSize;
    return true;
}
...
bool cbc::computeCipherLen()
{
    ...
    if((m_iNumPlainBytes%m_iBlockSize)==0) {
        m_iNumCipherBytes= m_iNumPlainBytes+4*m_iBlockSize;
    }
    else {
        k= (m_iNumPlainBytes+m_iBlockSize-1)/m_iBlockSize;
        m_iNumCipherBytes= k*m_iBlockSize+3*m_iBlockSize; 
    }
    return true;
}
...
{
#ifdef CRYPTOTEST4
    PrintBytes((char*)"cbc::nextMac", puA, m_iBlockSize);
#endif
...
bool cbc::nextCipherMac(byte* puIn, byte* puOut)
{
    byte    oldX[MAXAUTHSIZE];

#ifdef CRYPTOTEST4
    fprintf(g_logFile, "cbc::nextCipherMac\n");
    PrintBytes((char*)"In", puIn, 16);
#endif
...
bool cbc::firstCipherBlockIn(byte* puIn)
{
    ...
    PrintBytes((char*)"IV", puIn, m_iBlockSize);
#endif
    memcpy(m_rguFirstBlock, puIn, m_iBlockSize);
    memcpy(m_rgLastBlock, puIn, m_iBlockSize);
    return true;
...
bool cbc::firstCipherBlockOut(byte* puOut)
{
    ...
    PrintBytes((char*)"IV", m_rgLastBlock, m_iBlockSize);
#endif
...
#ifdef CRYPTOTEST1
    PrintBytes((char*)"cbc::lastPlainBlockIn\n", puIn, size);
#endif
...
// last three or four blocks
{
...
#ifdef CRYPTOTEST1
    PrintBytes((char*)"last cipher block after decode\n", 
    ...
    for(residue=(m_iBlockSize-1); residue>=0; residue--) {
        if(m_rguLastBlocks[residue]!=0) {
            if(m_rguLastBlocks[residue]!=0x80) {
                fprintf(g_logFile, "CBC bad pad error, %02x, res: %d size: %d\n", 
    ...
    if(residue<0) {
        fprintf(g_logFile, "CBC bad pad error\n");
        return -1;
    }
    m_iNumPlainBytes-= m_iBlockSize-residue;
