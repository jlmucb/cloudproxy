...
VMM_STATUS msr_vmexit_bits_config(UINT8 *p_bitmap, MSR_ID msr_id, RW_ACCESS access, BOOLEAN set);
static BOOLEAN  msr_common_vmexit_handler(GUEST_CPU_HANDLE gcpu, RW_ACCESS access, 
                    UINT64 *msr_value);
static BOOLEAN  msr_unsupported_access_handler(GUEST_CPU_HANDLE gcpu, MSR_ID msr_id, 
...
static BOOLEAN  msr_efer_write_handler(GUEST_CPU_HANDLE gcpu, MSR_ID msr_id, 
...
static BOOLEAN  msr_feature_control_read_handler(GUEST_CPU_HANDLE gcpu, 
                                MSR_ID msr_id, UINT64 *msr_value, void *context);
static BOOLEAN  msr_feature_control_write_handler(GUEST_CPU_HANDLE gcpu, 
                            MSR_ID msr_id, UINT64 *msr_value, void *context);
static BOOLEAN  msr_mtrr_write_handler(GUEST_CPU_HANDLE gcpu, MSR_ID msr_id, 
                                       UINT64 *msr_value, void *context);
static BOOLEAN  msr_vmcs_resident_default_handler(GUEST_CPU_HANDLE gcpu, 
                        MSR_ID msr_id, RW_ACCESS access, UINT64 *msr_value);
static BOOLEAN  msr_misc_enable_write_handler(GUEST_CPU_HANDLE gcpu, MSR_ID msr_id, 
                                            UINT64 *msr_value, void *context);
...
MSR_VMEXIT_DESCRIPTOR * msr_descriptor_lookup( LIST_ELEMENT *msr_list, MSR_ID msr_id)
{
...
static void msr_vmexit_register_mtrr_accesses_handler(GUEST_HANDLE guest) {

    UINT32 i,msr_addr;

#ifdef JLMDEBUG
    bprint("msr_vmexit_register_mtrr_accesses_handler\n");
#endif
...
// Allocates structures for MSR virtualization
...
void msr_vmexit_guest_setup(GUEST_HANDLE guest)
{
    ...
    vmexit_install_handler(guest_get_id(guest), vmexit_msr_read,  
                           Ia32VmxExitBasicReasonMsrRead);
    vmexit_install_handler(guest_get_id(guest), vmexit_msr_write, 
                           Ia32VmxExitBasicReasonMsrWrite);
...
// Register MSR related structures with HW (VMCS)
// ARGUMENTS: GUEST_CPU_HANDLE gcpu
// RETURNS  : none, must succeed.
...
// Register specific MSR handler with VMEXIT
// GUEST_HANDLE        guest
// MSR_ID              msr_id
// MSR_ACCESS_HANDLER  msr_handler,
// RW_ACCESS           access
// RETURNS  : VMM_OK if succeeded
VMM_STATUS msr_vmexit_handler_register( GUEST_HANDLE guest, MSR_ID msr_id,
                MSR_ACCESS_HANDLER  msr_handler, RW_ACCESS access, void *context)
{
...
#ifdef JLMDEBUG
    if(msr_id==0x1b)
        bprint("msr_vmexit_handler_register 0x1b\n");
#endif
...
// Unregister specific MSR VMEXIT handler
// GUEST_HANDLE  guest
// MSR_ID        msr_id
...
// Read handler which calls upon VMEXITs resulting from MSR read access
//          : Read MSR value from HW and if OK, stores the result in EDX:EAX
// GUEST_CPU_HANDLE gcp
...
// Write handler which calls upon VMEXITs resulting from MSR write access
...
// If MSR handler is registered, call it, otherwise executes default
...
// Try to execute real MSR read/write
// If exception was generated, inject it into guest
// ARGUMENTS: GUEST_CPU_HANDLE    gcpu
//          : MSR_ID              msr_id
//          : RW_ACCESS           access
// RETURNS  : TRUE if instruction was executed, FALSE otherwise (fault occured)
BOOLEAN msr_trial_access( GUEST_CPU_HANDLE gcpu, MSR_ID  msr_id,
                RW_ACCESS access, UINT64 *msr_value)
{
...
BOOLEAN msr_vmcs_resident_default_handler( GUEST_CPU_HANDLE gcpu, MSR_ID msr_id,
            RW_ACCESS access, UINT64 *msr_value)
{
    VMCS_OBJECT *vmcs = gcpu_get_vmcs(gcpu);
...
// Inject General Protection /fault event into the GCPU
...
BOOLEAN msr_unsupported_access_handler( GUEST_CPU_HANDLE gcpu, MSR_ID msr_id UNUSED,
                    UINT64 *value  UNUSED, void  *context UNUSED)
{
    REPORT_MSR_WRITE_ACCESS_DATA msr_write_access_data;

#ifdef JLMDEBUG
    bprint("msr_unsupported_access_handler\n");
#endif
    msr_write_access_data.msr_id = msr_id;
    // Using write access method for both read/write access here
    if (!report_uvmm_event(UVMM_EVENT_MSR_WRITE_ACCESS, 
...
// Handle guest access to EFER. Update guest visible value.
...
#pragma warning( push )
#pragma warning (disable : 4100)  // Supress warnings about unreferenced formal parameter
static BOOLEAN msr_efer_update_is_gpf0(GUEST_CPU_HANDLE gcpu, UINT64 new_value) {
    IA32_EFER_S efer;
...
#ifdef JLMDEBUG
    bprint("msr_efer_write_handler, msr: %08x\n", msr_id);
#endif
    VMM_ASSERT(IA32_MSR_EFER == msr_id);
    msr_write_access_data.msr_id = msr_id;
    if (!report_uvmm_event(UVMM_EVENT_MSR_WRITE_ACCESS, 
            (VMM_IDENTIFICATION_DATA)gcpu, (const GUEST_VCPU*)guest_vcpu(gcpu), 
            &msr_write_access_data))
        return FALSE;
...
{
#ifdef JLMDEBUG
    bprint("msr_efer_read_handler, msr: %08x\n", msr_id);
#endif
...
BOOLEAN msr_pat_read_handler( GUEST_CPU_HANDLE gcpu, MSR_ID  msr_id,
            UINT64  *msr_value, void *context UNUSED)
{
#ifdef JLMDEBUG
    bprint("msr_pat_read_handler\n");
#endif
    VMM_ASSERT(IA32_MSR_PAT== msr_id);
...
// Track Guest writes to Loacal APIC Base Register
// GUEST_CPU_HANDLE  gcpu
//          : MSR_ID            msr_id
//          : UINT64           *msr_value
// RETURNS  : TRUE, which means that instruction was executed.
BOOLEAN msr_lapic_base_write_handler( GUEST_CPU_HANDLE gcpu, MSR_ID msr_id,
                UINT64 *msr_value, void *context UNUSED)
{
    REPORT_MSR_WRITE_ACCESS_DATA msr_write_access_data;

#ifdef JLMDEBUG
    bprint("msr_lapic_base_write_handler %x\n", msr_id);
#endif
...
// Handles MSR reads on FEATURE_CONTROL MSR (0x3A). 
...
// Handles writes to FEATURE_CONTROL MSR (0x3A). 
...
// Handles writes to MISC_ENABLE MSR (0x1A0).
// Blocks writes to bits that can impact TMSL behavior
// ARGUMENTS: GUEST_CPU_HANDLE  gcpu
//          : MSR_ID            msr_id
//          : UINT64           *msr_value
// RETURNS  : TRUE, which means that instruction was executed.
BOOLEAN msr_misc_enable_write_handler( GUEST_CPU_HANDLE gcpu, MSR_ID msr_id,
        UINT64 *msr_value, void *context UNUSED)
{
    REPORT_MSR_WRITE_ACCESS_DATA msr_write_access_data;
...
// Install VMEXIT handler which prevents access to MSR from the guest
// ARGUMENTS: GUEST_HANDLE    guest
//          : MSR_ID      msr_id
// RETURNS  : VMM_OK if succeeded
VMM_STATUS msr_guest_access_inhibit( GUEST_HANDLE guest, MSR_ID msr_id)
{
    return msr_vmexit_handler_register( guest, msr_id, 
               msr_unsupported_access_handler, READ_WRITE_ACCESS, NULL);
}
