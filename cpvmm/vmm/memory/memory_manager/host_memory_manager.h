...
INLINE MAM_HANDLE hmm_get_hva_to_hpa_mapping(HMM* hmm) {
    return hmm->hva_to_hpa_mapping;
}

INLINE void hmm_set_hva_to_hpa_mapping(HMM* hmm, MAM_HANDLE mapping) {
    hmm->hva_to_hpa_mapping = mapping;
}

INLINE MAM_HANDLE hmm_get_hpa_to_hva_mapping(HMM* hmm) {
    return hmm->hpa_to_hva_mapping;
}

INLINE void hmm_set_hpa_to_hva_mapping(HMM* hmm, MAM_HANDLE mapping) {
    hmm->hpa_to_hva_mapping = mapping;
}

INLINE UINT64 hmm_get_current_vmm_page_tables(HMM* hmm) {
    return hmm->current_vmm_page_tabless_hpa;
}

INLINE void hmm_set_current_vmm_page_tables(HMM* hmm, UINT64 value) {
    hmm->current_vmm_page_tabless_hpa = value;
}

INLINE VMM_LOCK* hmm_get_update_lock(HMM* hmm) {
    return &(hmm->update_lock);
}

INLINE HVA hmm_get_new_allocations_curr_ptr(HMM* hmm) {
    return hmm->new_allocations_curr_ptr;
}

INLINE void hmm_set_new_allocations_curr_ptr(HMM* hmm, HVA new_value) {
    hmm->new_allocations_curr_ptr = new_value;
}

#ifdef DEBUG
INLINE UINT64 hmm_get_final_mapped_virt_address(HMM* hmm) {
    return hmm->final_mapped_virt_address;
}
#endif

INLINE void hmm_set_final_mapped_virt_address(HMM* hmm, UINT64 addr) {
    hmm->final_mapped_virt_address = addr;
}

INLINE UINT32 hmm_get_wb_pat_index(HMM* hmm) {
    return hmm->wb_pat_index;
}

INLINE void hmm_set_wb_pat_index(HMM* hmm, UINT32 wb_pat_index) {
    hmm->wb_pat_index = wb_pat_index;
}

INLINE UINT32 hmm_get_uc_pat_index(HMM* hmm) {
    return hmm->uc_pat_index;
}

INLINE void hmm_set_uc_pat_index(HMM* hmm, UINT32 uc_pat_index) {
