...
static
UINT32 safe_value_to_string (
    char*       buffer,
    UINT32      buffer_limits, //total space in out buff
    INT64       value,
    UINT32      flags,
    UINT32      width,
    UINT32      base,
...
--*/
{
    ...
    do
    {
        ...
        *(temp_str++) = chars[remainder];
        ...
        {
            if ((count % 3 == 0) &&
                (MAX_NUMBER_CHARS>(count+comma_count+pre_count))) {
...
static
UINT32 safe_value_to_decimal_str (
    char*   buffer,
    UINT32  buffer_limits, //total space in out buff
    INT64   value,
    UINT32  flags,
  ...
  )
{
    ...
    return prefix_size + safe_value_to_string(
                                         buffer,
                                         buffer_limits,
                                         value,
                                         flags | UNSIGNED,
                                         width,
                                         16,
                                         (flags & UPHEX) ? up_hex : low_hex );
}

static
UINT32 safe_time_to_string (
  VMM_TIME*     time,
  char*         buffer,
  UINT32        buffer_size
  )
...
static
const char* get_flags_and_width_and_precision (
  const char*     format,
  UINT32*   flags,
  UINT32*   width,
  UINT32*   precision,
    ...
    for (index = 0; (*format != '\0') && (available_chars > 0); ++format)
    {
        ...
        else
        {
            ...
            switch (*format)
            {
                ...
                case 't':   /* VMM_TIME* */
                    chars_written  = safe_time_to_string (
                              va_arg (argptr, VMM_TIME *),
                              &buffer[index],
                              available_chars
                              );
                    if (chars_written == 0)
